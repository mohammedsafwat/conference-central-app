...
# - - - Conference objects - - - - - - - - - - - - - - - - - - -
    ...
    def getConferenceSessions(self, request):
        ...
        return SessionForms(
            items=[self._copySessionToForm for session in sessions]
        )
    ...
    @endpoints.method(SPEAKER_GET_REQUEST, SessionForms,
        path='sessions/{speaker}', http_method='GET', name='getSessionBySpeaker')
    def getSessionBySpeaker(self, request):
        """Return all sessions given by a certain speaker"""
        data = {field.name: getattr(request, field.name) for field in request.all_fields()}
